...
task branch binding 的问题；不得减少任何检查、降低阈值、放宽 Git 绑定或扩大自动权限。
...
5. `tests/unit/test_policy.py`、`tests/unit/test_verification_plan.py` 和
...
6. `README.md`、`docs/operations/quickstart.md`、
   ...
   event head SHA，步骤不 fetch、不 push，也不改变 bootstrap path。
7. `.ai/tasks/TASK-0030/**` 仅由本任务正式生命周期维护。历史 state、task、evidence、
   approval 和 acceptance 文档中的 `2.1.0` 保持形成时事实，不得改写；当前 acceptance
   contract 可按第 5 项显式验证这些历史事实已相对 active Policy 失效。
...
3. workflow job 上限精确为 `90` 分钟，触发器、只读权限、required check 名称、锁定安装、
   ...
   switch 失败、最终 branch 不匹配或最终 SHA 漂移均立即失败；测试固定全部拒绝路径及步骤顺序。
...
6. 真实 PR 最终 head 的 required `ai-quality-gate` 成功；日志证明 checkout 的精确 event SHA
   ...
   diagnostics upload。push/PR 使用当前用户单独授权，merge 仍未授权。
...
attachment 未保持精确 event SHA、范围扩展、绑定失效、审查或批准陈旧、验证失败或 Gate
拒绝时必须停止并按 CLI 指引恢复；不得通过降为 V0、跳过 coverage、手改 evidence、放宽
branch equality、重复碰运气或恢复 bootstrap 来放行。
...
approval 和 GitHub Actions run `33403951577` 均保持追加式，不得删除、重排或改写为通过。
