...
- 真实 Linux PR CI 现在按 TASK-0025 原始 CRLF 工作树条件重放历史 snapshot；clean-checkout 在 uv 环境不含 pip 时使用锁定工具链中的 uv 离线安装，并把 Actions 的 detached checkout 投影为绑定同一 SHA 的本地测试分支；Quickstart 的 uv 依赖检查不再假定环境内存在 pip。
