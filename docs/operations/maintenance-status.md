...
| 现有 AI 工具接入 | 已准备[轻量接入指南](adoption.md)、ZCode 提示词、可合并规则及只读仓库盘点。2026-09-08 已在本仓完成一次受限的付费只读冒烟，观察记录见 [ZCode 单次只读接入观察](../../.ai/tasks/TASK-0047/zcode-readonly-pilot-observation.md)；这不是正常编码交付、完整引擎接入或效率/可靠度证明，外部项目与全局配置没有改变，真实编码任务仍待开展。完整引擎跨仓库初始化/定位/事实录入/CI 适配是条件性产品化工作，不是轻量试点前置，也不启动阶段三/四。 |
