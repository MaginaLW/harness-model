...
项目所有者已撤销本文的项目固定主模型、固定子模型及推理档位默认值，也撤销将特定模型
组合和旧版 backend 关系作为长期规则的做法。当前选择原则与角色边界以
[模型选择与角色边界](../../operations/model-selection.md)为准；主会话由当前会话的
模型选择控制，子智能体按当前运行时实际暴露的能力选择，并以真实线程元数据验证身份。
项目的每会话并发子智能体上限仍为 2。

本文以上内容保留为 2026-08-05 的决定、观察与验证记录，不代表当前配置或当前运行时能力，
也不构成恢复旧模型默认值、旧档位或固定模型组合的指令。历史上的主模型负责统筹和最终
复核、子智能体负责独立工作等职责，应按当前指南中的角色理解，不绑定任何模型代际。
