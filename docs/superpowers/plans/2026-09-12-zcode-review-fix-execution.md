...
后续统一从[2026-09-12 计划收尾与执行顺序](2026-09-12-plan-closeout-and-next-steps.md)接手。
9 月 12 日新增了可信私有目标的 Windows runner PILOT_PASS 证据，但 Linux/POSIX 与 main
采用尚未完成。本目录以上分支、外仓状态和历史结果保留原观察时点；新 CI 不覆盖旧
业务提交，也不直接关闭全部 E1 或证明 E3 的双产品交接。I 基础设施线与本目录 E1–E5
可以按各自依赖推进，W→E 映射保持，不重复建设或重新编号。

## 9. 后续案例核定（2026-09-22）

dotfiles CI 归因文档修订已形成真实的 Codex 发现与修复 → ZCode 独立复核案例，
满足 E3 最小 guided 双产品验收；固定版本、证据及范围见
[实际应用记录](../../operations/pilot-evidence-review-2026-09-22.md#2026-09-22-后续修订已应用)。
E4 仍待具体接口缺口和明确选定范围；以上历史快照不改写为全部 E1 或后续阶段完成。
