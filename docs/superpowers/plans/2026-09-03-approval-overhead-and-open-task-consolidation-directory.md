...
它们不需要修改引擎、Policy 或 schema，已写入 `AGENTS.md`：

1. **规则 8 —— 决策单元按风险面拆分。** 30 个 REVIEW 单元中，几乎每个都把文档/测试改动与
   治理面改动捆在同一个单元，因而只能按其中最危险的部分定级。拆分即可让安全那半合法走
   AUTO。详见[第 5 节前的实测结论](#实测结论基于路径的-auto-放宽在本仓库无收益2026-09-05)。
2. **规则 5 更正 —— 不请求系统未要求的批准。** 原规则称「批准和证据必须绑定当前规格、
   Policy 与 `subject_commit`」，对 `spec_approval` 不成立（其绑定为
   `(base_commit, policy_sha256, spec_sha256)`）。Agent 依此散文行事，造成 48 条重复批准中
   **11 条（23%）系统从未要求过**。

### 一句话结论

**审批开销的来源不是 Policy 太严，是决策单元太粗，以及一份写错的说明文档。**
48 条重复批准中 31 条（65%）源于实践而非引擎要求；30 个兜底 REVIEW 中可由任何路径规则
挽回的是 0 个。
